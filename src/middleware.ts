...
import { isPublicRoute, isOnboardingRoute, isAuthRoute, hasCompletedOnboarding, getOnboardingStep } from '@/lib/onboarding-helpers'
...
export async function middleware(request: NextRequest) {
  ...
  try {
    ...
    const { data: profile } = await supabase
      .from('users')
      .select('whatsapp_validated, business_type, content_interest, summary_frequency, subscription_status')
      .eq('id', user.id)
      .single()

    // If user hasn't completed onboarding and not on onboarding route
    if (profile && !hasCompletedOnboarding(profile) && !isOnboardingRoute(pathname)) {
